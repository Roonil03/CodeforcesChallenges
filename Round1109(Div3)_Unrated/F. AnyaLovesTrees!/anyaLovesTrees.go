...
func solve() {
	...
	var dfs func(u int) (int, int, int, bool)
	dfs = func(u int) (int, int, int, bool) {
		if len(adj[u]) == 0 {
			return arr[u], arr[u], 1, true
		}
		a, b, sz := math.MaxInt, math.MinInt, 0
		m := make([]int, 0, len(adj[u]))
		for _, v := range adj[u] {
			c1, c2, csz, fg := dfs(v)
			if !fg {
				return 0, 0, 0, false
